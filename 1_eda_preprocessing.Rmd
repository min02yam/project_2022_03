...
tmp_test %>% group_split(case)  %>%
  lapply(function(x){na_interpolation(x, option="linear")}) -> imp_list_test
...
# 6. 저장
```{r}
DT_scaled %>% write.csv("train_final.csv", row.names=FALSE)
DT_test_scaled %>% write.csv("test_final.csv", row.names=FALSE)
